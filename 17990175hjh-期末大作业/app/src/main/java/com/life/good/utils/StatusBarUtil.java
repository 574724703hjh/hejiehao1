...
public class StatusBarUtil {
    ...
    public static float DEFAULT_ALPHA = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ? 0 : 0.3f;
    ...
    public static void darkMode(Activity activity, boolean dark) {
        ...
        } else if (isMIUI6Later()>=6) {
            darkModeForMIUI6(activity.getWindow(), dark,isMIUI6Later());
    ...
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (isFlyme4Later()) {
            ...
            darkModeForMIUI6(window, true,isMIUI6Later());
            immersive(window,color,alpha);
        }   else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
...
//        window.setStatusBarColor(Color.TRANSPARENT);
     ...
     */
    public static boolean darkModeForMIUI6(Window window, boolean darkmode,int version) {
        if (version>=9){
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
    ...
    public static boolean isFlyme4Later() {
        return Build.FINGERPRINT.contains("Flyme_OS_4")
                ...
                || Pattern.compile("Flyme [4|5|6|7]", Pattern.CASE_INSENSITIVE).matcher(Build.DISPLAY).find();
    }

    /** 判断是否为MIUI6以上 */
    public static int isMIUI6Later() {
        try {
